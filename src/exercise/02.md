...
const getInitialState = () => Number(window.localStorage.getItem('count')) const
[count, setCount] = React.useState(getInitialState).

this is an example of lazy initialization. When passing a scalar (i.e. string,
number, boolean) to the state as the initial value, it is a low stakes
computation. When passing a ref (object i.e. function or array), this is
expesnive, so we don't want it called each re render (i.e. when state is
updated) so we pass it into the initial state and thus it will only get called
on mount.
